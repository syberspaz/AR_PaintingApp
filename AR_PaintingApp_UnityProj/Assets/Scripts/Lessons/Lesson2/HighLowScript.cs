...
{
    public class HighLowScript : MonoBehaviour
    {
            ...
            for (int i = 0; i < ScannedCamTex.width; i++)
            {
                for (int j = 0; j < ScannedCamTex.height; j++)
                {
                    ...
                    if (checkHighkey)
                    {
                        if (val <= 0.5)
                        {
                            //ScannedCamTex.SetPixel(i, j, Color.HSVToRGB(hue, sat, 0.08f));
                            ScannedCamTex.SetPixel(i, j, Color.red);
                        }
                    }
                    else
                    {
        ...
        Texture2D ConvertToGrayscale()
        {
            Texture2D grayTex = new Texture2D(CamTex.width, CamTex.height);
            for (int i = 0; i < CamTex.width; i++)
            {
                for (int j = 0; j < CamTex.height; j++)
                {
                    Color32 thisCol = CamTex.GetPixel(i, j);
                    int p = ((256 * 256 + thisCol.r) * 256 + thisCol.b) * 256 + thisCol.g;
                    int b = p % 256;
